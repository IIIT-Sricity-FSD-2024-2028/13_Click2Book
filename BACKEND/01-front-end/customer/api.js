...
<<<<<<< HEAD:front-end/customer/api.js

  tracking: {
    /** Current location for an in-progress trip (requires a booking on it) */
    async getLocation(tripId) {
      const customerId = getCustomerId();
      return apiFetch(`/tracking/${tripId}?customerId=${encodeURIComponent(customerId)}`);
    },
    /** Estimated arrival for an in-progress trip */
    async getEta(tripId) {
      return apiFetch(`/tracking/${tripId}/eta`);
    },
  },

  emergency: {
    /** Raise an SOS alert on a confirmed, in-progress trip */
    async raise(bookingId, type, message = '', lat = null, lng = null) {
      const customerId = getCustomerId();
      return apiFetch('/emergency', {
        method: 'POST',
        body: JSON.stringify({
          bookingId, customerId, type, message,
  ...
  lostFound: {
    /** Report a lost item against a trip/booking */
    async report(tripId, bookingId, itemDescription, category, dateLost, contactPhone) {
      const customerId = getCustomerId();
      return apiFetch('/lost-found', {
        method: 'POST',
        body: JSON.stringify({ tripId, bookingId, customerId, itemDescription, category, dateLost, contactPhone }),
      });
    },
    /** Get the current customer's own reports */
    async getMine() {
      const customerId = getCustomerId();
      return apiFetch(`/lost-found/customer/${customerId}`);
    },
  },
=======
>>>>>>> origin/main:BACKEND/01-front-end/customer/api.js
};
